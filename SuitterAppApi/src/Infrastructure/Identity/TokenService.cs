...
using SuitterAppApi.Application.Common.Exceptions;
using SuitterAppApi.Application.Identity.Tokens;
...
using SuitterAppApi.Shared.Multitenancy;
...
internal class TokenService : ITokenService
{
    ...
    public async Task<TokenResponse> GetTokenAsync(TokenByMobileRequest request, string ipAddress, CancellationToken cancellationToken)
    {
        var use = await userService.GetByMobileAsync(request.Phone, cancellationToken);

        if (string.IsNullOrWhiteSpace(_currentTenant?.Id)
            || await _userManager.FindByIdAsync(use.Id.ToString()) is not { } user
            || !await _userManager.CheckPasswordAsync(user, request.Password))
        {
            throw new UnauthorizedException(_t["Authentication Failed."]);
        }
        ...
        if (!user.IsActive)
        {
            throw new UnauthorizedException(_t["User Not Active. Please Confirm Otp."]);
        }

        if (_securitySettings.RequireConfirmedAccount && !user.PhoneNumberConfirmed)
        {
            throw new UnauthorizedException(_t["Phone Number not confirmed."]);
        }
    ...
    public async Task<TokenResponse> RefreshTokenAsync(RefreshTokenFromMobileRequest request, string ipAddress)
    {
        var userPrincipal = GetPrincipalFromExpiredToken(request.Token);
