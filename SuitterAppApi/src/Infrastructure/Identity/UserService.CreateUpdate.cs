...
using SuitterAppApi.Application.Common.Mailing;
using SuitterAppApi.Application.Identity.Users;
using SuitterAppApi.Domain.Common;
using SuitterAppApi.Domain.Identity;
...
internal partial class UserService
{
    ...
    public async Task<string> CreateByPhoneAsync(CreateUserByPhoneRequest request, string origin)
    {
        var user = new ApplicationUser
        {
        ...
        {
            await SendMsg(user.PhoneNumber, user.Otp);
        }


        return string.Join(Environment.NewLine, messages);
    }
    async Task SendMsg(string to, string msg)
    {
        var url = "https://api.ultramsg.com/instance42152/messages/chat";
        var client = new RestClient(url);

        var request = new RestRequest(url, Method.Post);
        request.AddHeader("content-type", "application/x-www-form-urlencoded");
        request.AddParameter("token", "dqw2lbsfiq1sld9t");
    ...
    public async Task<bool> ConfirmViaOtp(string phone, string otp)
    {
        var user = await _userManager.FindByNameAsync(phone.ToString());
        if (user != null)
        {
    ...
    public async Task<bool> ConfirmViaOtpFromWhats(string mobileNumber, string otp)
    {
